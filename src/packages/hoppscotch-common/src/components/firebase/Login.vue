<template>
  ...
  >
    ...
    <template #footer>
      ...
      <div v-if="mode === 'email' || mode === 'password-signin' || mode === 'password-signup'">
        <HoppButtonSecondary
...
const signUpWithPassword = async () => {
  ...
  try {
    await platform.auth.signUpWithPassword?.(form.email, form.password)
    hideModal()
  } catch (e: any) {
    toast.error(e?.response?.data?.message ?? e?.message ?? "Sign up failed")
  } finally {
